...
import { ChevronDown, Lock, LockOpen, Search } from "lucide-react";
...
import type { DocExpansion } from "@/apispector-config";
import {
...
export function EndpointList({
  ...
  useEffect(() => {
    if (docExpansion === "full") {
      setExpandedTags(new Set(controllers));
    } else if (docExpansion === "none") {
      setExpandedTags(new Set());
    } else {
      setExpandedTags(controllers.length > 0 ? new Set([controllers[0]]) : new Set());
    }
  }, [controllers.join(","), docExpansion]);

  if (endpoints.length === 0) {
    return (
      <div className="flex h-full min-h-0 flex-col items-center justify-center px-6 py-12 text-center">
        <p className="text-sm font-medium text-foreground">No endpoints discovered</p>
        <p className="mt-2 max-w-sm text-xs text-muted-foreground leading-relaxed">
          Ensure your app exposes @RestController handlers and that{" "}
          <code className="rounded bg-muted px-1 py-0.5 font-mono text-[11px]">
            apispector.scan.base-packages
          </code>{" "}
          includes your controller package. See{" "}
          <a
            href="https://github.com/apispector/api-spector/blob/main/docs/CONSUMER.md"
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto">
        ...
        {filtered.length === 0 ? (
          ...
          controllers.map((controller) => {
            const isOpen = expandedTags.has(controller);
            return (
            <Collapsible
              key={controller}
              open={isOpen}
            ...
            >
              <CollapsibleTrigger asChild>
                <button
                  type="button"
                  className="sticky top-0 z-1 flex w-full items-center gap-2 border-b border-border bg-muted/50 px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground backdrop-blur-sm hover:bg-muted/70"
                >
                  <ChevronDown
                    className={cn(
